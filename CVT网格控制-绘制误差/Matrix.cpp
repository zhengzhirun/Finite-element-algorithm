...
double PDE::f(const Point& p)
{
	double value;
    /*
	value = - 2 * PI * PI * exp(PI * (p[0] + p[1])) * (sin(PI * p[0]) * cos(PI * p[1]) + cos(PI * p[0]) * 
            sin(PI * p[1]));
	*/
    value = 2 * PI * PI * sin(PI * p[0]) * sin(PI * p[1]);
    return value; 
}
...
double PDE::u_exact(const Point& p)
{
	...
	*/
    value = sin(PI * p[0]) * sin(PI * p[1]);
    return value;
}

std::vector<double> PDE::u_exact_grad(const Point& p)
{
    std::vector<double> val(2);
    /*
    val[0] = PI * exp(PI * (p[0] + p[1])) * cos(PI * p[0]) * sin(PI * p[1]) +
        PI * exp(PI * (p[0] + p[1])) * sin(PI * p[0]) * sin(PI * p[1]);
    val[1] = PI * exp(PI * (p[0] + p[1])) * cos(PI * p[1]) * sin(PI * p[0]) + 
        PI * exp(PI * (p[0] + p[1])) * sin(PI * p[0]) * sin(PI * p[1]);
    */
    val[0] = PI * cos(PI * p[0]) * sin(PI * p[1]);
    val[1] = PI * cos(PI * p[1]) * sin(PI * p[0]);
    return val;
}
...
double Matrix::ComputerH1Error(const std::vector<double> &f)
{
   double err=0.0;
   int n_ele=mesh.n_element();
   for(int j=0; j<n_ele; j++){
        ...
        for(int i = 0; i < q_point.size(); i++){
            ...
            std::vector<double> f_value = pde.u_exact_grad(q_point[i]);
