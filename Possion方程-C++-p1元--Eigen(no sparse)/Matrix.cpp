...
void Matrix::SolveLinearA(std::vector<std::vector<double>>& A_matrix, std::vector<double>& u, std::vector<double>& rhs, int k)
{
    ...
    // 将vector<double>类型的数据容器转换到vector中
    ...
    switch(k){
        case 1:
        // 利用BDCSVD技术求解代数方程组
        // Requsivements on the matrix: none
        // speed (small-to-medium): -
        // speed (large) : -
        // Accurary: +++
        //
            u_h_now = A.bdcSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(b);
            break;
        case 2:
        // Requsivements on the matrix: none
        // speed (small-to-medium): -
        ...
        // speed (large): ++
        // accuracy: +
            u_h_now = A.householderQr().solve(b);
            break;
        case 4:
        // Requsivements on the matrix: none
        // speed (small-to-medium):+
        // speed (large): -
        // accuracy : +++
            u_h_now = A.colPivHouseholderQr().solve(b);
            break;
        case 5:
        // Requsivements on the matrix: none
        // speed (small-to-medium):-
        // speed (large): --
        // accuracy: +++
            u_h_now = A.fullPivHouseholderQr().solve(b);
            break;
        case 6:
        // LU分解
        // Requsivements on the matrix: Invertible
        // speed (small-to-medium): ++
        // speed (large): ++
        // accuracy: +
            u_h_now = A.partialPivLu().solve(b);
            break;
        case 7:
        // Requsivements on the matrix: None
        // speed (small-to-medium): -
        // speed (large): --
        // accuracy: +++
            u_h_now = A.fullPivLu().solve(b);
            break;
        case 8:
        // Requivements on the matrix: Positive definite
        // speed (small-to-medium): +++
        // speed (large): +++
        // accuracy: +
            u_h_now = A.llt().solve(b);
            break;
        case 9:
        // Requivements on the matrix: Positive or negative semidefinite
        // speed (small-to-medium): +++
        // speed (large): +
        // accuracy: ++
